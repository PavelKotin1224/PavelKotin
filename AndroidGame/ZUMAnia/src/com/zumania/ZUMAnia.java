...
public class ZUMAnia extends Activity {
	...
	public class GameHandler extends Handler {
		 ...
		 * @param delay
		 */
		public void addInitMessage(long delay) {
		...
		/**
		 * Add refresh message of surface
		 * @param delay
		 */
		public void addRefreshSurfaceMessage(long delay) {
			Message msg = new Message();
			msg.what = MESSAGE_ID;

			msg.arg1 = HANDLE_SURFACE_EVENT;
			sendMessageDelayed(msg, delay);
		}
		 ...
		 * @param delay
		 */
		public void addRefreshTimeEventMessage(long delay) {
			Message msg = new Message();
			msg.what = MESSAGE_ID;
		...
		public void removeAllMessages() {
			removeMessages(GameHandler.MESSAGE_ID);
		}
		...
		public void handleMessage(Message msg) {
			...
			SurfaceViewRepaintListener surfaceViewListener;
			boolean msgAdded = false;

			synchronized (activity) {
				if (gameData.gameStatus == GAME_QUIT)
					return;

				try {
					synchronized (this) {
						if (gameData.gameStatus == GAME_PAUSED)
							return;

						if (gameData.currPage != null) {
							page = gameData.currPage.page;
							timeEventListener = page.getTimeEventListener();
							surfaceViewListener = page.getSurfaceViewRedrawListener();

							if (surfaceViewListener != null) {

								// Process Time Event
								if (timeEventListener != null) {
									if (msg.arg1 == HANDLE_TIME_EVENT) {
										// Call time event
										gameData.teUpdateCount++;
										timeEventListener.onTimeEvent(gameData.teUpdateCount);
										if (page != gameData.currPage.page)
											return;
									}

									// Add time event
									addRefreshTimeEventMessage(page.getTimeEventInterval());
									msgAdded = true;
								}

								// Process Redraw Event of surface
								if (surfaceViewListener != null) {
									if (msg.arg1 == HANDLE_SURFACE_EVENT) {
										// Call surface event
										gameData.svUpdateCount++;
										gameCanvas.draw(surfaceViewListener, gameData.svUpdateCount);
										if (page != gameData.currPage.page)
											return;
									}

									// Add surface event
									addRefreshSurfaceMessage(page.getSurfaceViewRedrawInterval());
									msgAdded = true;
								}
							}
						}
